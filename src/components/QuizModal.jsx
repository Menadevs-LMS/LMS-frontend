...
const quizSchema = z.object({
  title: z.string()
    .min(3, { message: "Quiz title is required" })
    .max(100, { message: "Quiz title must be less than 100 characters" }),
  questions: z.array(questionSchema)
    .min(1, { message: "Add at least one question to the quiz" })
});
...
const QuizModal = ({ 
  ...
  console.log("QuizModal - Lecture ID:", lectureId);
  console.log("QuizModal - Initial Quiz Data:", initialQuizData);
  ...
  useEffect(() => {
    if (initialQuizData) {
      console.log("Initializing quiz form with data:", initialQuizData);
      
      // Ensure the initialQuizData has the correct structure
  ...
  }, [initialQuizData, reset, isOpen]);

  // Calculate if add button should be disabled
  const isAddButtonDisabled = () => {
    if (!currentQuestion.questionText) return true;
    
    if (currentQuestion.questionType === 'multiple-choice') {
      // Check if at least 2 options are filled and one is selected as correct
      const filledOptions = currentQuestion.options.filter(opt => opt.trim().length > 0);
      return filledOptions.length < 2 || !currentQuestion.correctAnswer;
    }
    
    if (currentQuestion.questionType === 'truth-sentence') {
      // Check if at least one truth answer is provided
      return !currentQuestion.truthAnswers.length;
    }
    
    // For paragraph questions, only question text is required
    return false;
  };

  // Handle question input change
  const handleQuestionInputChange = (e, optionIndex) => {
    const { name, value } = e.target;
    
    if (name === 'options' && optionIndex !== undefined) {
    ...
    } else if (name === 'truthAnswers') {
      // Handle truth answers as comma-separated values
      setCurrentQuestion(prev => ({
        ...prev,
        truthAnswers: value.split(',').map(item => item.trim()).filter(item => item)
      }));
    } else {
  ...
  const addQuestionToQuiz = () => {
    ...
    try {
      // Create a cleaned question object based on the question type
      let newQuestion = {
      ...
      } else if (currentQuestion.questionType === 'truth-sentence') {
        newQuestion.truthAnswers = currentQuestion.truthAnswers;
      } else if (currentQuestion.questionType === 'paragraph') {
        newQuestion.paragraphAnswer = currentQuestion.paragraphAnswer || '';
      }
      
      console.log("Adding question to quiz:", newQuestion);
  ...
  const onSubmit = (data) => {
    ...
    // Save the quiz data and close the modal
    handleAddQuiz(lectureId, data);
    reset();
    onClose();
  };
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          ...
          {watch('questions')?.length > 0 && (
            <div className="mt-6">
              ...
              {errors.questions && (
                <p className="mt-2 text-sm text-red-500">{errors.questions.message}</p>
              )}
            </div>
          )}
          ...
          <div className="mt-8 flex gap-3">
            ...
            <button
              type="submit"
              disabled={watch('questions')?.length === 0 || !watch('title')}
              className={`flex-1 px-4 py-2.5 font-medium rounded-lg transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-green-400 ${
                watch('questions')?.length === 0 || !watch('title')
                  ? 'bg-gray-200 text-gray-500 cursor-not-allowed dark:bg-gray-700 dark:text-gray-400'
